...
class BiencoderCollator(DataCollatorWithPadding):
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> BatchEncoding:
        ...
        merged_batch_dict['q_task_input_ids'] = torch.tensor([0,1,2,3]).unsqueeze(0).repeat(len(query_examples),1)
        merged_batch_dict['d_task_input_ids'] = torch.tensor([0,1,2,3]).unsqueeze(0).repeat(len(doc_examples), 1)
