...
def _worker_encode_passages(gpu_idx: int):
    ...
    dataset = load_dataset('json', data_files=args.encode_in_path, cache_dir='./.cache')['train']
    if args.dry_run:
    ...
    for batch_dict in tqdm.tqdm(data_loader, desc='passage encoding', mininterval=8):
        batch_dict['task_input_ids']=torch.tensor([0,1,2,3]).unsqueeze(0).repeat(batch_dict['input_ids'].size(0), 1)
